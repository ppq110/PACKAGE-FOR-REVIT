...
AUTH_SERVER_URL="${AUTH_SERVER_URL:-http://192.168.110.213:5050}"
BIND_URL="${BIND_URL:-http://0.0.0.0:5050}"
...
cat > "$ENV_FILE" <<EOF
export DYNLOCK_AUTH_SERVER_URL="$AUTH_SERVER_URL"
export DYNLOCK_AUTH_SERVER_ADMIN_EMAIL="$SUPER_ADMIN_EMAIL"
export DYNLOCK_AUTH_DATABASE_PROVIDER="$DATABASE_PROVIDER"
export DYNLOCK_AUTH_DATABASE_CONNECTION_STRING="$POSTGRES_CONNECTION_STRING"
export DYNLOCK_AUTH_SERVER_BIND_URL="$BIND_URL"
EOF
...
echo
echo "If firewalld is enabled, open port 5050:"
echo "  sudo firewall-cmd --add-port=5050/tcp --permanent"
